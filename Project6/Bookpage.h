...
public:
    ...
    void showNotifications() {
        auto notifications = currentUser->getNotifications();
        ...
        for (const auto& noti : notifications) {
            cout << "◆ " << noti.getMessage() << endl;

            if (noti.getType() == Notification::RESERVATION_AVAILABLE) {
                reservedBooks.push_back(noti.getBookIsbn());
            }
        }
        // 处理预约确认
        if (!reservedBooks.empty()) {
            ...
            if (choice == 'y' || choice == 'Y') {
                for (const auto& isbn : reservedBooks) {
                    cout << 1;
                    confirmborrow(isbn, currentUser->getUsername());
                }
            }
        }
    }
